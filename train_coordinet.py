    ...
    for epoch in range(start_epoch, opt.epochs + 1):
        ...
        for i, (imgs, poses) in enumerate(dataloader):
            ...
            if steps % opt.save_freq == 0:
                ...
                angle_diff = np.median(np.hstack(angle_diffs)) / np.pi * 180
                t_diff = np.median(np.hstack(t_diffs))
                print('[valid] [epoch:%d, median_diff]t_diff:%.4fm, angle_diff:%.4f°' % (epoch, t_diff, angle_diff))
                ...
                ##########################
                ########## save ##########
                loger.log_loss({'valid/median_angle_diff': angle_diff, 'valid/median_t_diff': t_diff}, epoch, steps, epoch_steps)
